...
public class BudgetPlanDao {
    ...
    public List<BudgetPlan> getBudgetAll() {
        logger.debug("..getBudgetAll");
        List<BudgetPlan> bpList = null;
        PreparedStatement pstm = null;
        ResultSet rs = null;
        try {
            conn = new DbConnection().open();
            ...
            sql.append(" SELECT `budp_id`, `budp_name`, `budp_year_begin`, `budp_year_end`, ");
            sql.append(" `modified_date`, `modified_by` ");
            sql.append(" FROM `budget_plan` ORDER BY budp_name ASC");

            pstm = conn.prepareStatement(sql.toString());
            logger.info("pstm ::==" + pstm.toString());
            rs = pstm.executeQuery();
            bpList = new ArrayList<BudgetPlan>();
            while (rs.next()) {
                bpList.add(getEntityBudgetPlan(rs));
            }
        } catch (Exception e) {
            logger.error("Error getBudgetAll :", e);
        }
        return bpList;
    }
